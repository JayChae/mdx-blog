...
export function SidebarContainer({ children, className }: ClassNameProp) {
  const { open, pinned, setOpen, hasMouse } = useSidebar()
  ...
  const baseClassName = cn(
    ...
    !hasMouse ? 'w-full' : pinned ? 'w-64' : 'w-60',
    className,
  )

  const props = {
    className: baseClassName,
    ...(hasMouse && { onMouseLeave: handleMouseLeave }),
  }
...
export function SidebarPinButton({ children, className }: ClassNameProp) {
  const { setPinned, hasMouse } = useSidebar()
  const toggle = () => {
    setPinned((prev) => !prev)
  }
